...
## STR_SA_POSTFIX_LIST <- c("notimeout", "maxitems", 
##                          "attempt4", "attempt7", "attempt10", "attempt13", 
##                          "recent_active_5", "recent_active_10", "recent_active_15", 
##                          "total_active_30", "total_active_60", "total_active_90") 

## change to get different files
STR_SA_POSTFIX_LIST <- c("notimeout_avg", "minitems", "maxitems", 
                         "attempt4_avg", "attempt7_avg", "attempt10_avg", "attempt13_avg", 
...
for(STR_SA_POSTFIX in STR_SA_POSTFIX_LIST) {
    s <- NULL
    for (STR_REGION in STR_REGION_ALL) {
        ...
        ## s6 is the main model
        s <- cbind(s, process.one.model(s7))
    }

    ## write file
    str.outfile <- sprintf("./%s/%s_%d_%s.csv", STR_RESULTS_DIR, STR_OUT_PREFIX, 7, STR_SA_POSTFIX)
